...
export class VidLinkScraper {
  ...
  static async getStream(
    ...
    try {
      console.log(
        `[VidLink] Generating token for ${type} ${tmdbId}${type === "tv" ? ` S${season}E${episode}` : ""}...`,
      );
      ...
      while (retryCount < MAX_RETRIES) {
        try {
          ...
          console.warn(
            `[VidLink] API attempt ${retryCount} failed: ${err.message}. Retrying...`,
          );
      ...
      try {
        ...
        console.error(
          `[VidLink] Failed to parse API response as JSON. Status: ${response.statusCode}`,
        );
        console.error(
          `[VidLink] Raw body (first 200 chars): ${String(response.body || "").substring(0, 200)}`,
        );
      ...
      const captionsSubtitles =
        data.stream.captions?.map((c: any) => {
      ...
      if (subtitles.length > 0) {
        console.log(
          `[VidLink] Found ${subtitles.length} subtitles for ${tmdbId} (English: ${subtitles.some((s) => s.lang === "en" || s.languageName.toLowerCase().includes("english"))})`,
        );
      } else {
        console.warn(
          `[VidLink] No subtitles found in API response for ${tmdbId}`,
        );
      }
      ...
      console.log(
        `[VidLink] Successfully extracted ${mirrors.length} mirrors (cookies: ${cookies ? "YES" : "NONE"}).`,
      );
      return mirrors;
    } catch (e: any) {
