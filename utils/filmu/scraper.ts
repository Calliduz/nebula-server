...
export class FilmuScraper {
  ...
  public static async getStream(params: {
    ...
    try {
      ...
      // ── Task E: Bingr.one Providers (Miller, Mann, Edmunds, Aditya) ──
      const srvs = ["s1", "s2", "s3", "s5"];
      srvs.forEach((srv) => {
        tasks.push(
          (async () => {
            try {
              ...
              const subtitles = Array.isArray(data?.subtitles)
                ? data.subtitles.map((s: any) => ({
                    ...
                    languageName: s.label || s.languageName || "English",
                    source: `FilmU-Bingr-${scraperName}`,
                  }))
                : [];
              ...
              console.warn(
                `[FilmU Scraper] Bingr.${srv} extraction failed:`,
                err.message,
              );
              return [];
            }
