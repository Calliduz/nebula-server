...
function loadKeys(): KeyConfig {
  try {
    ...
    throw new Error(
      "KissKH decryption keys not found. See docs/KISSKH_KEYS.md",
    );
  }
}
...
export function decryptKissKHSubtitle(rawSrt: string, fileUrl: string): string {
  ...
  console.log(
    `[KISSKH/DECRYPT] Processing ${lines.length} lines with method ${methodName}...`,
  );
  ...
  console.log(
    `[KISSKH/DECRYPT] ✔ Decrypted ${decryptedCount} cues, ${failedCount} skipped/failed`,
  );
  return output.join("\n");
}
