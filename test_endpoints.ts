...
test("Download APIs", async (t) => {
  await t.test("GET /api/download for movie (Fight Club - 550)", async () => {
    ...
    console.log(
      `Found ${data.torrents.length} torrents for Fight Club. First source: ${torrent.source}`,
    );
  });

  await t.test(
    "GET /api/download/episode for TV show (Game of Thrones - 1399, S1E1)",
    async () => {
      const res = await fetch(
        `${BASE_URL}/api/download/episode?tmdbId=1399&season=1&episode=1`,
      );
      assert.equal(res.status, 200);
      const data = (await res.json()) as any;
      ...
      assert.ok(
        data.torrents.length > 0,
        "Should find backup episode torrents for GoT S1E1",
      );
      ...
      assert.ok(
        torrent.source,
        "Episode torrent should have source provider label",
      );

      console.log(
        `Found ${data.torrents.length} backup streams for Game of Thrones S1E1. First source: ${torrent.source}`,
      );
    },
  );
});
