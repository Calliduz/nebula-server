...
import { VaplayerScraper } from "../utils/vaplayer.js";

// ── Router factory ────────────────────────────────────────────────────────────
export function createVaplayerRouter(): Router {
  const router = Router();
   ...
   */
  router.get("/api/vaplayer", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (
        ...
        const vaplayerMirrors = (cachedRecord.mirrors as any[]).filter(
          (m: any) =>
            typeof m.source === "string" && m.source.startsWith("Vaplayer"),
        );

        if (
          vaplayerMirrors.length > 0 &&
          (!cachedRecord.streamExpiresAt ||
            new Date() < cachedRecord.streamExpiresAt)
        ) {
          console.log(
            `[VAPLAYER] Cache HIT ✔ for ${tmdbId} S${season}E${episode} (${vaplayerMirrors.length} mirrors)`,
          );
          return res.json(buildResponseObject(vaplayerMirrors));
        }
      }
      ...
      const runScan = async () => {
        try {
          const mirrors = await VaplayerScraper.getStream({
            tmdbId,
            kind: type,
            season,
            episode,
          ...
          if (firstMirror) {
            ...
            if (existingRecord && existingRecord.mirrors) {
              // Merge: keep non-Vaplayer mirrors, replace/add Vaplayer ones
              const nonVaplayerMirrors = (
              ...
              const mergedMirrors = [...nonVaplayerMirrors, ...mirrors];
              ...
              await StreamCache.findOneAndUpdate(
                { tmdbId: tmdbId.toString(), type, season, episode },
                {
                  ...
                  source: firstMirror.source || "Vaplayer",
                  qualityTag: "HD",
          ...
          console.warn(
            `[VAPLAYER] Scan failed for ${tmdbId} S${season}E${episode}: ${err.message}`,
          );
          fetchFinished = true;
          return null;
        }
      ...
      console.error("[VAPLAYER] Route error:", error.message);
      return res
        .status(500)
        .json({ error: "Failed to fetch from Vaplayer providers" });
    }
  });
